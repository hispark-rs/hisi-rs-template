...
//! binary must opt in itself with `-Thisi-riscv-link.x`. hisi-riscv-rt exposes its
//! OUT_DIR (containing `hisi-riscv-link.x` + the scripts it INCLUDEs) via
...
//! `hisi-riscv-link.x`'s `INCLUDE memory.x` resolves to THIS file (exactly one on the
//! path → no ambiguity).
use std::env;
use std::fs::File;
use std::io::Write;
